...
import os
x=os.getcwd()
rf_data=pd.read_csv(x+'/Sub_Division_IMD_2017.csv')
...
full=l_state_dfs
for i in range(len(states)):
    train=int(len(l_state_dfs[i])*0.7)
    l_state_dfs[i]=l_state_dfs[i].head(int(len(l_state_dfs[i])*(80/100)))
    mean = cal_mean(l_state_dfs[i].value)
    ...
    covariance_of_rainfall_and_years = l_state_dfs[i].cov(min_periods=train)['YEAR']['value']
    w1 = covariance_of_rainfall_and_years / float(years_variance)
